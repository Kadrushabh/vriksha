...
async function createOrder(order) {
  ...
  console.log('SR payload:', JSON.stringify(payload));
  try {
    const res = await axios.post(`${BASE}/orders/create/adhoc`, payload, { headers: headers(token) });
    console.log('SR response:', JSON.stringify(res.data));
    return res.data;
  } catch(e) {
    console.error('SR createOrder error:', JSON.stringify(e?.response?.data || e.message));
    throw e;
  }
}
